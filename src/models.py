...
def RCNN(x, location, size, mask, labels, c_num, batch_size, is_train, reuse, conv_factory_used=conv_factory):
    with tf.variable_scope('C', reuse=reuse) as vs:
        ...
        with tf.variable_scope('RPN', reuse=reuse):
            ...
            with tf.variable_scope('cls', reuse=reuse):
                ...
                with tf.variable_scope('loss', reuse=reuse):
                    ...
                    x_cls = tf.sigmoid(x_cls)
                    chosen = tf.argmax(tf.reshape(x_cls, [batch_size, -1, 1]), axis=1)
                    print('chosen', chosen)
                    x_cls = tf.cast(tf.round(x_cls), tf.int64)
            ...
            with tf.variable_scope('reg', reuse=reuse):
                ...
                with tf.variable_scope('loss', reuse=reuse):
                    ...
                    chosen_reg = tf.gather_nd(tf.reshape(x_reg, [batch_size, -1, 3]), tf.concat(
                        [tf.expand_dims(tf.range(batch_size, dtype=tf.int64), 1), chosen[:, 0:1]], axis=1))
        ...
        theta = tf.transpose(tf.convert_to_tensor(
            [chosen_reg[:, 2] / 48, tf.constant(0, tf.float32, shape=[batch_size]), (chosen_reg[:, 0] - 24) / 24,
             tf.constant(0, tf.float32, shape=[batch_size]), chosen_reg[:, 2] / 48, (chosen_reg[:, 1] - 24) / 24]))
    ...
    variables = tf.contrib.framework.get_variables(vs)
    return loss, loss_cls, loss_reg, loss_cls_final, x_feat, accuracy, accuracy_final, variables, x_cls, cropped_images
