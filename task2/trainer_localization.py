...
class Trainer(object):
    ...
    def train(self):
        for step in trange(self.start_step, self.max_step):
            fetch_dict = {
                ...
                'loss_cls_final': self.loss_cls_final,
                'accuracy': self.accuracy,
                'accuracy_final': self.accuracy_final,
                'x_cls': self.x_cls,
            ...
            if step % self.log_step == self.log_step - 1:
                fetch_dict.update({
                    ...
                    'test_summary': self.test_summary_op})
            ...
            if step % self.log_step == self.log_step - 1:
                ...
                loss_cls_final = result['loss_cls_final']
                accuracy = result['accuracy']
                accuracy_final = result['accuracy_final']

                print(
                    "\n[{}/{}:{:.6f}] Loss_cls: {:.6f} Loss_reg: {:.6f} loss_cls_final: {:.6f} Accuracy: {:.4f} accuracy_final: {:.4f}". \
                    format(step, self.max_step, lr, loss_cls, loss_reg, loss_cls_final, accuracy, accuracy_final))
                sys.stdout.flush()
            ...
            if step % self.save_step == self.save_step - 1:
                ...
                for iter in np.arange(self.test_iter):
                    fetch_dict = {"test_accuracy": self.test_accuracy, 'loss_reg': self.test_loss_reg,
                                  'test_accuracy_final': self.test_accuracy_final}
                    result = self.sess.run(fetch_dict)
                    test_accuracy += result['test_accuracy']
                    test_regression += result['loss_reg']
                    test_accuracy_final += result['test_accuracy_final']
                test_accuracy /= self.test_iter
                test_regression /= self.test_iter
                test_accuracy_final /= self.test_iter

                print("\n[{}/{}:{:.6f}] Test Accuracy: {:.4f} Loss_reg: {:.4f} test_accuracy_final: {:.4f}". \
                      format(step, self.max_step, lr, test_accuracy, test_regression, test_accuracy_final))
                sys.stdout.flush()
    ...
    def build_model(self):
        ...
        self.total_loss, self.loss_cls, self.loss_reg, self.loss_cls_final, feat, self.accuracy, self.accuracy_final, \
        self.c_var, self.x_cls, self.cropped_images = self.model(
            x, self.train_location_loader, self.train_size_loader, self.train_msk_loader, self.labels, self.c_num,
            self.batch_size, is_train=True, reuse=False, conv_factory_used=self.conv_block)
        ...
        self.summary_op = tf.summary.merge([
            ...
            tf.summary.image("cropped_images", self.cropped_images),
    ...
    def build_test_model(self):
        ...
        self.test_total_loss, self.test_loss_cls, self.test_loss_reg, self.test_loss_cls_final, self.test_feat, \
        self.test_accuracy, self.test_accuracy_final, var, self.test_x_cls, self.test_cropped_images = self.model(
            test_x, self.test_location_loader, self.test_size_loader, self.test_msk_loader, self.test_labels,
            self.c_num,
            self.batch_size_test, is_train=False, reuse=True, conv_factory_used=self.conv_block)
        ...
        self.test_summary_op = tf.summary.merge([
            tf.summary.image("test_cropped_images", self.test_cropped_images)])
